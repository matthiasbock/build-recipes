...
function container_setup()
{
  ...
  container_exec $container_name apt update \
    || { echo "Failed to update package list. Aborting."; container_setup_failed; exit 1; }
#  container_exec $container_name apt upgrade -y \
#    || { echo "Failed to upgrade packages. Aborting."; container_setup_failed; exit 1; }
  container_exec $container_name apt remove -y $(cat "$container_config_dir/uninstall.list" | sed -ze "s/\n/ /g") \
    || { echo "Failed to remove packages. Aborting."; container_setup_failed; exit 1; }
  container_exec $container_name apt install -y $(cat "$container_config_dir/install.list" | sed -ze "s/\n/ /g") \
